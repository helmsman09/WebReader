# News Reading Monorepo (with summaries + real PDF/TTS)

This monorepo contains:

- **Backend** (Express + MongoDB + BullMQ)
  - Real **PDF parsing** via `pdf-parse`
  - **Summaries** via OpenAI Chat (summary worker + /summary endpoint)
  - **TTS** via OpenAI Audio (TTS worker + /tts endpoint)
- **Dashboard** (React + Vite)
  - Page list with OG/meta cards
  - **Summary panel** with "Re-summarize" button
  - Tag editor, upload modal, block renderer
- **Mobile** (React Native / Expo skeleton)
  - Library / AddContent / PageDetail (with summary)
- **Chrome extension** (Vite + React)
  - Captures cleaned article text + images + audio tags + OG/meta
